...
function FanpagesTab({ accountId }) {
  const { data: fanpages = [], isLoading } = useQuery({
    ...
    queryFn: () =>
      api.get(`/accounts/${accountId}/fanpages`).then((r) => r.data),
  })
  ...
  if (fanpages.length === 0) {
    return (
      <div className="bg-white rounded-xl border border-gray-200 p-8 text-center">
        ...
        <p className="text-sm text-gray-500">No fanpages found for this account</p>
      </div>
    )
  }
        ...
        <tbody className="divide-y divide-gray-100">
          {fanpages.map((page) => (
            <tr key={page.id} className="hover:bg-gray-50">
              ...
              <td className="px-4 py-3 text-gray-500">{page.page_id}</td>
...
function GroupsTab({ accountId }) {
  const { data: groups = [], isLoading } = useQuery({
    ...
    queryFn: () =>
      api.get(`/accounts/${accountId}/groups`).then((r) => r.data),
  })
  ...
  if (groups.length === 0) {
    return (
      <div className="bg-white rounded-xl border border-gray-200 p-8 text-center">
        ...
        <p className="text-sm text-gray-500">No groups found for this account</p>
      </div>
    )
  }
        ...
        <tbody className="divide-y divide-gray-100">
          {groups.map((group) => (
            <tr key={group.id} className="hover:bg-gray-50">
              ...
              <td className="px-4 py-3 text-gray-500">{group.group_id}</td>
              <td className="px-4 py-3 text-gray-500">
                {group.member_count ?? '-'}
              </td>
              <td className="px-4 py-3 text-gray-500">
