...
export default function UserAISettings() {
  ...
  if (loadingUser) return <div className="flex justify-center py-12"><Loader2 className="w-8 h-8 animate-spin text-info" /></div>
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <div>
          <h2 className="text-lg font-semibold text-app-primary">API Key AI của bạn</h2>
          <p className="text-sm text-app-muted mt-1">Thêm API key riêng để ghi đè cài đặt hệ thống. Để trống = dùng mặc định.</p>
        </div>
        <button
                ...
                return (
                  <span
                    ...
                    className={`inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full ${
                      val.source === 'user'
                        ? 'bg-green-100 text-hermes border border-green-300'
                        : 'bg-app-elevated text-app-muted'
                    }`}
                  >
                    <span className={`w-2 h-2 rounded-full ${info?.color || 'bg-app-hover'}`} />
                    {info?.name || key}
                    {val.source === 'user' && <span className="font-medium">(của bạn)</span>}
                    {val.source === 'admin' && val.enabled && <CheckCircle size={10} className="text-hermes" />}
                  </span>
                )
              })}
            </div>
            ...
            <div key={key} className="bg-app-surface rounded shadow p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs bg-green-100 text-hermes px-2 py-0.5 rounded-full">Key riêng</span>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs text-app-muted mb-1">Model</label>
                  <select
                    value={ps.model || ''}
        ...
        <div>
          {!showAdd ? (
            <button
              ...
              className="flex items-center gap-2 text-sm text-info hover:text-blue-800 border border-dashed border-blue-300 rounded-lg px-4 py-2 w-full justify-center hover:bg-blue-50"
            >
              <Plus size={16} /> Thêm API key riêng
            </button>
          ) : (
            <div className="bg-app-surface rounded shadow p-4">
              <p className="text-sm font-medium text-app-primary mb-3">Chọn provider:</p>
              ...
              <button onClick={() => setShowAdd(false)} className="text-xs text-app-dim mt-2 hover:text-app-muted">Hủy</button>
            </div>
          )}
        </div>
      )}
