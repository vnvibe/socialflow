...
export default function ProxyManager() {
  ...
  const statusIcon = (status) => {
    if (status === 'active' || status === 'online') return <CheckCircle size={14} className="text-green-500" />
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-xl shadow p-4 mb-6">
        ...
        <p className="text-sm text-gray-500 mb-3">Enter one proxy per line in format: <code className="bg-gray-100 px-1 rounded text-xs">ip:port:username:password</code></p>
        <textarea
          value={bulkText}
        ...
        <table className="w-full">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-sm font-medium text-gray-600">Host:Port</th>
          ...
          <tbody className="divide-y">
            {proxies.map(proxy => (
              <tr key={proxy.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-sm font-mono text-gray-600">{proxy.host}:{proxy.port}</td>
                <td className="px-4 py-3">
                  <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full uppercase">{proxy.type || 'HTTP'}</span>
                </td>
                ...
                <td className="px-4 py-3 text-sm">
                  {proxy.speed ? (
                    <span className="flex items-center gap-1">
                      <Wifi size={12} className={proxy.speed < 500 ? 'text-green-500' : proxy.speed < 2000 ? 'text-yellow-500' : 'text-red-500'} />
                      {proxy.speed}ms
                    </span>
                  ) : '—'}
                </td>
            ...
            {proxies.length === 0 && <tr><td colSpan={8} className="px-4 py-8 text-center text-gray-400">No proxies configured</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
