...
function UserSettings() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-app-primary mb-2">Cài đặt</h1>
...
function ExecutorCard({ executor, isPreferred, bothOnline, onSelect, disabled }) {
  ...
  return (
    <div className={`flex items-center gap-4 rounded border-2 px-4 py-3 transition-all ${
      isPreferred
      ...
      <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${
        ...
        <Icon size={18} className={isExtension ? 'text-purple-600' : 'text-info'} />
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <span className="text-sm font-semibold text-app-primary">{label}</span>
          <span className="w-2 h-2 rounded-full bg-hermes shrink-0" title="Online" />
        </div>
        <p className="text-xs text-app-dim truncate">{sublabel}</p>
      </div>
        ...
        <button
          ...
          className={`shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
            isPreferred
              ...
              : 'bg-app-surface border border-app-border text-app-muted hover:border-blue-400 hover:text-info'
          } disabled:opacity-50`}
        >
          {disabled ? (
...
function UserAgentTab() {
  ...
  return (
    <div className="space-y-6">
      {/* Status header */}
      <div>
        <h2 className="text-lg font-semibold text-app-primary mb-4">Executor</h2>
      ...
      {/* Download Agent */}
      <div>
        <h2 className="text-lg font-semibold text-app-primary mb-4">Tải Desktop Agent</h2>
        <div className="bg-app-surface rounded shadow p-6 space-y-4">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-2">
            <div className="flex items-start gap-3">
              ...
              <span className="text-sm text-app-primary">Tải file <strong>SocialFlow Agent.exe</strong></span>
            </div>
            <div className="flex items-start gap-3">
              ...
              <span className="text-sm text-app-primary">Double-click để chạy — hiện ngay trên thanh taskbar</span>
            </div>
            <p className="text-xs text-info mt-2 pl-9">Lần đầu sẽ tự cài đặt Chromium (~150MB). Sau đó mở lại là chạy ngay.</p>
          </div>
        </div>
      </div>
    </div>
