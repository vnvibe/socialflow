...
function AITypeBadge({ type }) {
  if (!type) return <span className="text-xs text-app-muted">--</span>
  const config = {
    potential_buyer: { label: 'Khach', color: 'bg-green-100 text-hermes' },
    competitor:      { label: 'Doi thu', color: 'bg-red-100 text-red-700' },
    irrelevant:      { label: 'Khong lq', color: 'bg-app-elevated text-app-muted' },
...
function ScoreBadge({ score }) {
  if (score === null || score === undefined) return <span className="text-xs text-app-muted">--</span>
  const color = score >= 7 ? 'text-hermes' : score >= 4 ? 'text-yellow-600' : 'text-red-500'
...
const TARGET_STATUS = {
  ...
  done:     { color: 'bg-green-100 text-hermes' },
  failed:   { color: 'bg-red-100 text-red-700' },
  skip:     { color: 'bg-app-elevated text-app-muted' },
}

const FRIEND_STATUS = {
  sent:           { color: 'bg-blue-100 text-blue-700' },
  accepted:       { color: 'bg-green-100 text-hermes' },
  declined:       { color: 'bg-red-100 text-red-700' },
  cancelled:      { color: 'bg-orange-100 text-orange-700' },
  already_friend: { color: 'bg-app-elevated text-app-muted' },
}
...
export default function DataCenterSection({ campaignId, accountIds }) {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-lg font-bold text-app-primary">Data Center</h2>
      ...
      {subTab === 'targets' && (
        <div className="bg-app-surface rounded border border-app-border overflow-hidden">
          <div className="grid grid-cols-[1fr_100px_60px_70px_80px] gap-2 px-4 py-2.5 bg-app-base border-b text-xs font-semibold text-app-muted uppercase">
            <div>Target</div>
            <div>Nguon</div>
            <div>Score</div>
            <div>AI Type</div>
          ...
          {targets.length === 0 ? (
            <div className="text-center py-12 text-app-muted text-sm">Chua co target</div>
          ) : targets.map((t, i) => {
            const st = TARGET_STATUS[t.status] || TARGET_STATUS.pending
            return (
              <div key={t.id || i} className="grid grid-cols-[1fr_100px_60px_70px_80px] gap-2 px-4 py-2.5 border-b border-app-border text-sm items-center hover:bg-app-base transition-colors">
                <div className="truncate text-app-primary">{t.fb_user_name || t.fb_user_id || '?'}</div>
                <div className="truncate text-xs text-app-muted">{t.source_group_name || '-'}</div>
          ...
          {targetsData?.total > 50 && (
            <div className="px-4 py-2 text-xs text-app-muted bg-app-base border-b">
      ...
      {subTab === 'friends' && (
        <div className="bg-app-surface rounded border border-app-border overflow-hidden">
          <div className="grid grid-cols-[1fr_60px_70px_80px_100px] gap-2 px-4 py-2.5 bg-app-base border-b text-xs font-semibold text-app-muted uppercase">
            <div>Target</div>
            <div>Score</div>
            <div>AI Type</div>
            <div>Status</div>
          ...
          {friends.length === 0 ? (
            <div className="text-center py-12 text-app-muted text-sm">Chua co friend request</div>
          ) : friends.map((f, i) => {
            const st = FRIEND_STATUS[f.status] || FRIEND_STATUS.sent
            return (
              <div key={f.id || i} className="grid grid-cols-[1fr_60px_70px_80px_100px] gap-2 px-4 py-2.5 border-b border-app-border text-sm items-center hover:bg-app-base transition-colors">
                <div className="truncate text-app-primary">{f.target_name || f.target_fb_id || '?'}</div>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            <div className="bg-app-surface rounded border border-app-border p-3 text-center">
              <p className="text-xl font-bold text-app-primary">{leadsStats.total || 0}</p>
              <p className="text-[10px] text-app-muted uppercase">Total</p>
            </div>
            <div className="bg-app-surface rounded border border-app-border p-3 text-center">
              <p className="text-xl font-bold text-blue-600">{leadsStats.by_status?.discovered || 0}</p>
              <p className="text-[10px] text-app-muted uppercase">Discovered</p>
            </div>
            <div className="bg-app-surface rounded border border-app-border p-3 text-center">
            ...
            <div className="bg-app-surface rounded border border-app-border p-3 text-center">
              <p className="text-xl font-bold text-hermes">{leadsStats.by_status?.connected || 0}</p>
              <p className="text-[10px] text-app-muted uppercase">Connected</p>
            </div>
            <div className="bg-app-surface rounded border border-app-border p-3 text-center">
              <p className="text-xl font-bold text-hermes">{leadsStats.by_type?.potential_buyer || 0}</p>
              <p className="text-[10px] text-app-muted uppercase">Buyers</p>
            </div>
          </div>
          ...
          {/* Leads Table */}
          <div className="bg-app-surface rounded border border-app-border overflow-hidden">
            <div className="grid grid-cols-[1fr_60px_70px_90px_90px_90px] gap-2 px-4 py-2.5 bg-app-base border-b text-xs font-semibold text-app-muted uppercase">
              <div>Name / UID</div>
              <div>Score</div>
              <div>AI Type</div>
              <div>Status</div>
            ...
            {leadsLoading ? (
              ...
              return (
                <div key={l.id || i} className="grid grid-cols-[1fr_60px_70px_90px_90px_90px] gap-2 px-4 py-2.5 border-b border-app-border text-sm items-center hover:bg-app-base transition-colors">
                  <div className="min-w-0">
                    <p className="font-medium text-app-primary truncate">{l.name || l.fb_uid}</p>
                    <p className="text-xs text-app-muted truncate">{l.fb_uid}</p>
                  </div>
                  ...
                  <div className="text-xs text-app-muted truncate">{l.source || '-'}</div>
                  <div className="text-xs text-app-muted">
          ...
          {leadsData?.pages > 1 && (
            <div className="flex items-center justify-between">
              <span className="text-xs text-app-muted">Trang {leadsPage}/{leadsData.pages} ({leadsData.total})</span>
              <div className="flex gap-1">
                <button disabled={leadsPage <= 1} onClick={() => setLeadsPage(p => p - 1)}
                  className="px-2 py-1 text-xs bg-app-base border border-app-border text-app-dim rounded disabled:opacity-30">&laquo;</button>
                <button disabled={leadsPage >= leadsData.pages} onClick={() => setLeadsPage(p => p + 1)}
                  className="px-2 py-1 text-xs bg-app-base border border-app-border text-app-dim rounded disabled:opacity-30">&raquo;</button>
              </div>
            </div>
          )}
        </div>
