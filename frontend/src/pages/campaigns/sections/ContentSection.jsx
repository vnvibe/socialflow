...
const JOB_STATUS = {
  done:      { label: 'Done',    color: 'bg-green-100 text-hermes' },
  running:   { label: 'Running', color: 'bg-blue-100 text-blue-700' },
  pending:   { label: 'Pending', color: 'bg-yellow-100 text-yellow-700' },
  failed:    { label: 'Failed',  color: 'bg-red-100 text-red-700' },
  cancelled: { label: 'Cancelled', color: 'bg-app-elevated text-app-muted' },
}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold text-app-primary">Noi dung & Bai dang</h2>
        <a
          href="/content/new"
      ...
      {contents.length > 0 && (
        <div>
          <h3 className="text-sm font-semibold text-app-muted mb-3">Mau noi dung ({contents.length})</h3>
          <div className="grid gap-3">
            {contents.map(c => (
              <div key={c.id} className="bg-app-surface rounded border border-app-border p-4">
                <p className="text-sm text-app-primary line-clamp-2">{c.caption || '(Khong co caption)'}</p>
      ...
      {/* Publish History */}
      <div>
        <h3 className="text-sm font-semibold text-app-muted mb-3">Lich su dang ({history.length})</h3>
        <div className="bg-app-surface rounded border border-app-border overflow-hidden">
          <div className="grid grid-cols-[1fr_100px_80px_120px] gap-2 px-4 py-2.5 bg-app-base border-b text-xs font-semibold text-app-muted uppercase tracking-wider">
            <div>Noi dung</div>
            <div>Target</div>
            <div>Status</div>
            <div>Thoi gian</div>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12 text-app-muted text-sm">Chua co bai dang</div>
          ) : (
            history.slice(0, 50).map(h => {
              const statusCfg = JOB_STATUS[h.status] || JOB_STATUS.pending
              return (
                <div key={h.id} className="grid grid-cols-[1fr_100px_80px_120px] gap-2 px-4 py-2.5 border-b border-app-border hover:bg-app-base transition-colors items-center text-sm">
                  <div className="min-w-0">
                    <p className="text-app-muted truncate text-xs">{(h.final_caption || '').substring(0, 80) || '...'}</p>
                  </div>
                  <div className="text-xs text-app-muted truncate">{h.target_name || h.target_type || '?'}</div>
                  <div>
