module.exports = async (fastify) => {
  ...
  // KEYWORDS CRUD + /posts/:id toggle/delete removed in phase-3 streamline
  // (2026-04-19): Frontend doesn't call any of them. Scan keyword flow is
  // superseded by Hermes orchestrator's assign_job(type=campaign_discover_groups).
  ...
  // /monitor/groups CRUD removed in phase-3 streamline (no frontend callers).
  // Discovery now flows through fb_groups + campaign_groups junction via
  // Hermes orchestrator.
  ...
  // /engagement/summary + /engagement/check-now removed in phase-3
  // streamline (no frontend callers). Engagement snapshots still write via
  // the agent's check_engagement handler; only the ad-hoc trigger endpoints
  // were unused.
