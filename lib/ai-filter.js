...
Nếu không có nhóm nào liên quan, trả về: []`
  ...
  // Fallback: keyword matching (broader — split topic + add common variations)
  const topicWords = topic.toLowerCase().split(/[\s,]+/).filter(w => w.length >= 2)
  // Add common related terms
  const RELATED = {
    'vps': ['hosting', 'server', 'cloud', 'máy chủ', 'thuê'],
    'hosting': ['vps', 'server', 'web', 'domain', 'cloud'],
    'openclaw': ['openclaw', 'open claw', 'claw', 'ai', 'chatbot', 'llm'],
  ...
  console.log(`[AI-FILTER] Keyword fallback: ${fallback.length}/${groups.length} (keywords: ${[...expanded].join(', ')})`)
